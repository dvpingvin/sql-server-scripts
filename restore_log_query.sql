DECLARE @full_backup_n INT = 0 -- OFFSET: 0 - last full backup, 1 - previus full backup, etc ...
DECLARE @full_backup_start_date DATETIME

SELECT @full_backup_start_date = backup_start_date FROM msdb.dbo.backupset
WHERE database_name = DB_NAME() AND recovery_model = 'FULL' AND is_copy_only = 0 AND type = 'D'
ORDER BY backup_start_date DESC OFFSET @full_backup_n ROWS FETCH NEXT 1 ROWS ONLY;

SELECT @full_backup_start_date AS last_full_backup_start_date
...
WHERE s.database_name = DB_NAME() AND s.[type] = 'L' AND s.recovery_model = 'FULL' 
