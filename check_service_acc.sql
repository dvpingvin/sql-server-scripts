...
WHEN [servicename] LIKE 'SQL Server (%)' AND ([service_account] NOT LIKE 'NT SERVICE\%$' OR [service_account] NOT LIKE '%\%$') AND SERVERPROPERTY('IsClustered') = 0 AND SERVERPROPERTY('IsHADREnabled') = 0 AND SERVERPROPERTY ('InstanceName') IS NOT NULL  THEN 'Поменяйте УЗ на NT SERVICE\MSSQL$' + CONVERT (NVARCHAR(128), SERVERPROPERTY ('InstanceName'))
WHEN [servicename] LIKE 'SQL Server Agent (%)' AND ([service_account] NOT LIKE 'NT SERVICE\%$' OR [service_account] NOT LIKE '%\%$') AND SERVERPROPERTY('IsClustered') = 0 AND SERVERPROPERTY('IsHADREnabled') = 0 AND SERVERPROPERTY ('InstanceName') IS NOT NULL  THEN 'Поменяйте УЗ на NT SERVICE\SQLAGENT$' + CONVERT (NVARCHAR(128), SERVERPROPERTY ('InstanceName'))
